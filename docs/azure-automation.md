# Running meraki2tf on a Schedule via Azure Automation

This guide deploys the scheduled DR jobs on Azure Automation with the
Meraki API key in Azure Key Vault and every run's artifacts archived to
geo-redundant Blob Storage: the **weekly snapshot job** (discovery →
snapshot → snapshot-diff drift → coverage → alerts; the wrapper's
default) and the **monthly terraform rehearsal** (the same wrapper with
`--with-terraform`, which follows the export with the offline `--sync`
pipeline — kit regeneration and guarded import-only state
materialization). The primary path is a **Hybrid Runbook Worker**; an
**Azure Container Apps Job** alternative is covered at the end.
...
```
Azure Automation (schedules, job history)
        │  triggers weekly (snapshot) + monthly (--with-terraform)
        ▼
Hybrid Runbook Worker (Linux VM, persistent disk)
  deploy/azure/runbook.py
        ...
        │ 2. meraki2tf --dump-to snapshot.jsonl.gz --drift-baseline <last run>
        │    (+ offline --sync pipeline when --with-terraform)
...
- **Automation account** with a weekly schedule (snapshot job) and a
  monthly one (terraform rehearsal).
...
`resources.tf` baseline across rehearsals and carries the rotated
drift-baseline snapshot between weekly runs — do not put it on
ephemeral/temp storage, and keep it out of any cleanup jobs. Losing it does not lose
...
prefix, what that run refreshed: weekly snapshot runs upload
`snapshot.jsonl.gz`, `coverage.json`, `coverage.txt`, and `runbook.md`
(the DR runbook is regenerated by every run in every mode); monthly
`--with-terraform` runs additionally upload the kit — `resources.tf`,
`imports.tf`, and `provider.tf`. (Snapshot-only runs do not re-upload
the kit: whatever is on disk is the last rehearsal's output, and
archiving it under a fresh prefix would misdate it. Note also that a
snapshot-only run's `coverage.json` reports every kit-captured asset
as `pending-import` — it never reads Terraform state; the
state-aware imported/pending split is in the latest monthly prefix.)
The Terraform state is deliberately **not** placed in this artifact
prefix — it is secret-bearing. With the local backend it stays `0600` on the worker and
...
`--`) on the monthly `--with-terraform` job — the backend only matters
when the terraform stage runs. The managed identity authenticates via
AAD — no storage key on disk or in a flag:
...
python3 runbook.py ... --with-terraform -- \
...
(The wrapper adds `--dump-to` and `--sync` itself; never pass either
through `--` — the export stage would reject the combination.)
...
# Weekly snapshot job (the default shape). The wrapper rotates last
# run's snapshot in as the --drift-baseline automatically.
python3 runbook.py \
...
# Monthly terraform rehearsal: same parameters plus --with-terraform,
# which follows the export with the offline --sync pipeline.
python3 runbook.py \
  --vault-name kv-netops \
  --org-id 123456 \
  --workdir /var/lib/meraki2tf \
...
`--spec`, …) stays available — though `--confirm-deletions` and
`--rebaseline` act on the terraform stage and are refused (exit 2)
without `--with-terraform`, so a human decision is never silently
dropped. The wrapper exits with meraki2tf's own exit code (0 clean, 1
fault, 3 coverage gaps) and forces nonzero if artifact archival fails —
wire your Automation job alerts to nonzero completions.

Attach the schedules (one weekly, one monthly):
...
az automation schedule create --automation-account-name aa-netops \
  -g rg-netops --name monthly-first-0300 --frequency Month --interval 1 \
  --start-time 2026-08-01T03:00:00+00:00
```

then link the runbook to each schedule and the `meraki2tf-workers`
group in the portal (or via `az automation job-schedule create`),
passing `--with-terraform` in the monthly link's runbook parameters.

Two deployment notes:

- **Upgrade the CLI together with the runbook.** The snapshot-only
  shape passes `--fail-on-gaps` on the `--dump-to` invocation, which
  meraki2tf releases predating the demotion reject at argparse — an
  old CLI under the new runbook fails every weekly run with exit 2.
- **Overlap protection is built in.** Both schedules share one
  workdir; the wrapper holds a `flock` on
  `<workdir>/.meraki2tf-wrapper.lock` for the whole run and refuses to
  start (exit 1) while another run is active, so a long monthly
  rehearsal cannot have the weekly job rotate the snapshot out from
  under it. Stagger the schedules anyway — a refused run is a skipped
  snapshot.
...
- **Schedule:** cron triggers replace the Automation schedules — one
  job for the weekly snapshot (e.g. `0 6 * * 1`) and a second job with
  `--with-terraform` for the monthly rehearsal (e.g. `0 3 1 * *`).
...
1. Pull the latest `runs/<timestamp>/` prefix from
   ...
   meraki2tf installed. The freshest snapshot and `runbook.md` are in
   the latest weekly prefix; the latest full kit (`resources.tf`,
   `imports.tf`, `provider.tf`) is in the most recent
   `--with-terraform` (monthly) prefix.
