...
**Primary Disaster-Recovery Mission:** The owning team manages Meraki via clickops; meraki2tf is the safety net. Run on a schedule, it dumps **everything** out of the Meraki tenant into a restore-grade snapshot (weekly), and converts it into runnable Terraform artifacts with a materialized Terraform state (the monthly terraform rehearsal) — so the environment can be rebuilt after a major incident. Meraki (the dashboard) is the source of truth in this mode; Terraform trails it, never leads it.
...
1. **Meraki is never mutated.** No run — scheduled, ad-hoc, live, or dump — may ever change anything in the Meraki organization. The sole exceptions are the explicit, human-invoked disaster-recovery actions `--rebuild --confirm` (terraform apply of the kit), `--replay-gaps --confirm` (snapshot replay of unsupported objects and uncaptured secrets), `--restore --confirm` (full-organization rebuild from a snapshot, which additionally may only ever target `--target-org` — a named organization that is **never** the snapshot's source org), `--heal --confirm` (same-organization partial recovery: **additive-only** recreation of snapshot objects missing from live discovery — accidental deletions; surviving objects are never modified, `--org-id` must equal the snapshot's source org, and a sanitized snapshot is refused), and `--wipe-org --confirm` (drill-organization teardown, which is refused outright for any organization holding claimed devices and requires the exact org name as a second factor); each flag alone is a read-only preview.
...
The scheduled terraform rehearsal (weekly until the 2026-07-17 demotion, monthly since) must build real Terraform state unattended, which requires applying import blocks. This is permitted under a strict guard, because `import` blocks only write state — they never touch Meraki:
...
- Every auto-applied run must report exactly which resources were added to state (in the log and in the success notification), so scheduled reruns tell the operator what grew.
...
- **Deleted objects:** **alert-only.** Deletions are never silently synced out of the DR kit — an accidental clickops deletion must not quietly poison the rebuild baseline. A human reviews the alert and confirms removal (e.g. via `--rebaseline` or an explicit confirmation flag). Cadence note (post-demotion): the weekly snapshot-diff surfaces a deletion **once** (the next week's baseline already lacks the object); the persistent `DELETION_PENDING_CONFIRMATION` reminder lives in the kit-based flow and therefore re-fires on every terraform (monthly) run until confirmed.
...
- **`--sync` (opt-in DR mode):** enables the guarded import-only auto-apply and modified-object HCL regeneration described above. This is the flag the scheduled terraform rehearsal passes (monthly since the 2026-07-17 demotion; the weekly job is snapshot-only — `--dump-to` + `--drift-baseline`, plus `--fail-on-gaps` in the recommended shape).
...
### Drift-Race Handling at Scale (sync mode)
On a large, actively-administered org each plan round takes hours and new clickops edits land inside almost every window. Sync mode therefore (a) iterates the Meraki-is-truth regeneration up to a bounded number of rounds per run (each with its own DRIFT_DETECTED alert), and (b) when the heal budget is spent and everything still drifting is update-only on **not-yet-imported** resources, defers those few from the run's kit so the import-only remainder applies — monotone state growth; deferred objects are alerted (`deferred_addresses`) and import on the next run. Tracked-resource drift and non-update mutations are never deferred; the import-only apply guard is unchanged. When the full plan still carries update pressure, state is materialized through **targeted chunk windows** (`terraform plan -target` over ~1000 pending imports at a time): each chunk's saved plan independently passes the same import-only guard before applying, shrinking the race window from hours to minutes and making state growth monotone regardless of how busy the org is.

### Snapshot-First Evolution (2026-07-10, approved plan)
The weekly DR loop is evolving to snapshot-first (see `~/.claude/plans/enchanted-snacking-wirth.md`): parallel spec-driven discovery (incl. nested multi-parameter surfaces) → v2 stream snapshot → `--drift-baseline` snapshot-diff drift (spec-normalized, `origin: snapshot-diff` on DRIFT_DETECTED) → coverage manifest with a `restore_via` verdict per asset → alerts. The direct-API restore engine (`--restore --from-dump SNAP --target-org ORG [--confirm] [--serial-map PATH]`, wave-ordered, ID-journaled/resumable, reference-rewriting, preview-first) is the org-loss rebuild path. Drills: run `--restore --skip-claims` (hardware is mono-org — claims/device features are drill-skipped verdicts, not failures), prefer the **sanitized snapshot** so no real secrets/identifiers enter the drill org, and tear down with `--wipe-org ID --wipe-org-name NAME --confirm` (interlocks: refuses any org with claimed devices — production always has hardware; exact-name second factor; preview-first; re-verified at execution). **The drill gate is MET: the full-org restore drill into a scratch organization PASSED on 2026-07-14** (sanitized snapshot, `--skip-claims`, 0 failures, idempotent journal resume; the same-org `--heal` drill passed the same day). **Terraform demotion was decided and EXECUTED on 2026-07-17:** the scheduled weekly job is snapshot-only (the Azure wrapper defaults to the export stage, rotating last run's snapshot in as `--drift-baseline` automatically; `--fail-on-gaps` is valid on `--dump-to` runs so the weekly coverage gate holds), and the terraform pipeline runs as the **monthly rehearsal** (`--with-terraform` in the wrapper, or a direct `--sync` run) alongside the `--rebuild` preview — it remains the proven same-org subset-restore tool and the monthly plan catches provider regressions. Any change that would regress restore must re-pass a scratch-org drill before merging. **The full terraform pipeline (kit generation, `--sync` state materialization, guarded applies) is a permanent, first-class capability regardless of the weekly-loop decision — it is the supported transition path to managing Meraki as code.** None of the earlier surface may regress.

### Implementation Status Note
The full contract above is implemented as of 2026-07-02: `--sync` guarded auto-apply (guard in `terraform_runner.apply_import_plan`), the deletion flow (`DELETION_PENDING_CONFIRMATION` alert + `--confirm-deletions`), the coverage manifest (`coverage.json`/`coverage.txt`), and `--fail-on-gaps` (exit 3). As of 2026-07-03 the DR gap surface is implemented too: the per-run `runbook.md` (redacted payloads + replay operations + secret re-entry pointers, regenerated every run in `runbook.py`) and the guarded `--replay-gaps` action (`replayer.py`; preview by default, `--confirm` writes unsupported objects and dump-sourced secrets back via the SDK with name-based network-ID remapping, `GAP_REPLAY_EXECUTED` alert). Secrets at rest: unsanitized snapshots and the Terraform state are written/kept owner-only (0600); artifacts and alerts never carry secret values. Also as of 2026-07-03, state may live in a pluggable Terraform backend (`--state-backend`, with `azurerm` as the first remote target alongside the default `local`; settings via `--backend-config`/`--backend-config-file`, credentials env/MSI-only): the local read path is unchanged, remote state is read via `terraform show -json`, and `init` self-runs once per run — see `config.BackendConfig` and `terraform_runner`. As of 2026-07-14 (PRs #67–#69, proven by the passed drill): the restore engine adopts Meraki-provisioned defaults and org-shared built-ins instead of duplicating them (natural-key + sibling-mapping matching, sentinel-ID refresh), runs an end-of-run salvage retry for same-wave sibling 400s, the sanitizer emits drill-valid URLs and keeps built-in payload-template names, `--wipe-org` removes drill-created non-caller admins before org deletion, and a sanitized-snapshot `--restore --confirm` refuses any non-empty target org (the pseudonymized source-org ID defeats the self-org guard otherwise). None of it — nor the earlier surface (read-only pipeline, `--rebuild --confirm`, `--rebaseline`, dump/live providers, webhook/email alerts) — may regress.
...
- No run may **ever** mutate the Meraki organization. The only paths that touch Meraki are the explicit disaster-recovery actions `--rebuild --confirm`, `--replay-gaps --confirm`, `--restore --confirm`, `--heal --confirm`, and `--wipe-org --confirm`; each flag alone must remain a read-only preview, and none may ever run as part of a scheduled/pipeline invocation.
...
- **Secret-Bearing Artifacts:** Exactly two artifacts may hold secret values — the unsanitized snapshot and the Terraform state — and both must be written/kept owner-only (0600). Everything else (HCL kit, coverage manifest, runbook, alerts, logs, replay results) carries attribute names and locators, never values; the gap replayer reads secrets from the snapshot at execution time and holds them only in memory. When state lives in a **remote backend** (`--state-backend azurerm`, …) there is no local state file: at-rest protection is delegated to the backend (Storage-Account encryption + RBAC), the 0600 rule then applies only to the snapshot, and backend credentials must come from the environment/managed identity — never from a `--backend-config` flag (credential-shaped keys are refused so no secret reaches argv or logs).
