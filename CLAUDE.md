...
1. **Meraki is never mutated.** No run — scheduled, ad-hoc, live, or dump — may ever change anything in the Meraki organization. The sole exceptions are the explicit, human-invoked disaster-recovery actions `--rebuild --confirm` (terraform apply of the kit), `--replay-gaps --confirm` (snapshot replay of unsupported objects and uncaptured secrets), and `--restore --confirm` (full-organization rebuild from a snapshot, which additionally may only ever target `--target-org` — a named organization that is **never** the snapshot's source org); each flag alone is a read-only preview.
